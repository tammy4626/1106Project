...
int main(int argc, char **argv) {
    ...
    sort(elements.begin(), elements.end(), [&] (const Element &a, const Element &b) -> bool { return make_pair(a.x1, a.y1) < make_pair(b.x1, b.y1); }); // sort all elements from up to down, from left to right
    vector<Element> rows;
    ...
    for (auto r : rows) r.rows_beautify();
